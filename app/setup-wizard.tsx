              ...
              <>
                ...
                {activeStep === 3 ? (
                  <section className="wizard-step-card">
                    ...
                    {done[3] ? <><ConnectedCard title={bridge?.telegramAccount?.displayName || "Telegram hesabı"} detail={`${bridge?.telegramAccount?.username ? `@${bridge.telegramAccount.username} · ` : ""}Şifreli oturum (SESSION_ENCRYPTION_KEY)`} /><button className="primary-button wizard-next" onClick={() => setActiveStep(4)}>Business bağlantısına geç</button></> : (
                      <>
