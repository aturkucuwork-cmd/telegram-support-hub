...
|---|---|---|
| `AGENTS.md` | OpenAI Codex, Cursor, Gemini CLI, Aider, diğerleri | Universal standart; tek AI olarak tüm rolleri sırayla oynamayı anlatır |
...
- **Subagent destekli (Claude Code, opencode):** 7 subagent + 1 primary (Barış) ayrı konuşmalardır. Her biri kendi dosyasına yazar, paralel koşarlar. Ajans-workflow **gerçek çoklu-ajan** deneyimidir.
- **Tek AI (Codex, Copilot, Cursor, Gemini, Aider):** Aynı AI oturumu tüm rolleri sırayla oynar. Her rol geçişinde **"Şimdi X rolüne geçiyorum"** der, çıktıyı uzmana atanan dosyaya yazar. Ajans-workflow **sıralı simülasyon** olarak çalışır.
